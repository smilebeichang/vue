...
<script>
import { detail, update, remove } from '@/api/base/permissions'
...
export default {
  ...
  computed: {
    // 弹窗名字
    pageTitle () {
      return this.formBase.id ? '编辑权限' : '新建权限'
    },
    treeData () {
  ...
  methods: {
    ...
    handleCheckChange (data, checked, indeterminate) {
      ...
      // 用map数组方法筛选出id属性作为一个全新的数组
      this.permissions = this.treeCheckedNodes.map((item) => item.id)
    },
    // 表单提交
    async handleAdd (object) {
      try {
        ...
        if (this.formBase.id) {
          // 编辑更新数据
          await update({ ...this.formBase, permissions: this.permissions })
          Message.success('编辑成功')
          this.$parent.list()
          this.$emit('update:dialogFormVisible', false)
        } else {
          // 添加权限
          this.$emit('addPermissios', this.formBase.title, this.permissions)
        }
      } catch (error) {
        console.log(error)
      }
